...
cfg = {
    ...
    'test_deep':[32,32,'M',32,32,32,'M',32,32,32,'M',32,32,32,'M',32,32,32,32,32,32,32,32,'M'],
    }
    ...
    def _make_layers(self, cfg, quantize_bit=32):
        ...
        for x in cfg:
            ...
            if x == 'M':
                ...
                layers += [nn.AvgPool2d(kernel_size=2, stride=2),catSNN.Clamp_q(max = self.clamp_max,q_level=T_s)]
            else:
                ...
                #layers += [catSNN.QuantizedConv2d(in_channels, x, kernel_size=3, padding=1, bias=self.bias, quantize_bit=quantize_bit),nn.BatchNorm2d(x),
                            #catSNN.Clamp_q(max = self.clamp_max),nn.Dropout2d(0.3)]
                layers += [catSNN.QuantizedConv2d(in_channels, x, kernel_size=3, padding=1, bias=self.bias, quantize_bit=quantize_bit),nn.BatchNorm2d(x),catSNN.Clamp_q(max = self.clamp_max,q_level=T_s),nn.Dropout2d(0.2)]
        ...
        for x in cfg:
            ...
            if type(x) is tuple:
                layers += [nn.Conv2d(x[0], x[1], kernel_size=2, stride = 2, bias=self.bias, groups = x[1]),
                           catSNN.Clamp_q(max = self.clamp_max,q_level=T_s)]
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x == 'M':
                ...
                #        self.snn.spikeLayer()]
                layers += [self.snn.conv(in_channels, x, kernelSize=3, padding=1, bias=self.bias),
                        self.snn.spikeLayer(0.999),nn.Dropout2d(0)]
                in_channels = x
        return nn.Sequential(*layers)
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x == 'M':
                ...
                # catSNN.Clamp_q(max = self.clamp_max,q_level =1000)  catSNN.Clamp(max = self.clamp_max)
                layers += [catSNN.QuantizedConv2d(in_channels, out_channels, kernel_size=3, padding=padding, bias=self.bias),nn.BatchNorm2d(out_channels),catSNN.Clamp_q(max = self.clamp_max,q_level=T_s),nn.Dropout2d(0.15)]
...
class CatVGG_o(nn.Module):
    ...
    def _make_layers(self, cfg, is_noise=False):
        ...
        for x in cfg:
            if x == 'M':
                layers += [self.snn.pool(2),self.snn.spikeLayer(0.9999)]
            else:
                if is_noise:
                    layers += [self.snn.mcConv(in_channels, x, kernelSize=3, padding=1, bias=self.bias),
                               self.snn.spikeLayer(1.0),nn.Identity()]
                    in_channels = x
                else:
                    padding = x[1] if isinstance(x, tuple) else 1
                    out_channels = x[0] if isinstance(x, tuple) else x
                    layers += [self.snn.conv(in_channels, out_channels, kernelSize=3, padding=padding, bias=self.bias),
                               self.snn.spikeLayer(0.9999),nn.Identity()]
                    in_channels = out_channels
        return nn.Sequential(*layers)


class VGG_o_test_deep(nn.Module):
    def __init__(self, vgg_name, quantize_factor=-1, clamp_max=1.0, bias=True):
        super(VGG_o_test_deep, self).__init__()
        self.clamp_max = clamp_max
        self.bias = bias
        self.features = self._make_layers(cfg[vgg_name])
        self.classifier1 = nn.Linear(32, 10, bias=True)
        self.features.apply(initialize_weights)
    ...
    def _make_layers(self, cfg):
        ...
        for x in cfg:
            if x == 'M':
                layers += [nn.AvgPool2d(kernel_size=2, stride=2),nn.Dropout2d(0.1) ]
            else:
                padding = x[1] if isinstance(x, tuple) else 1
                out_channels = x[0] if isinstance(x, tuple) else x
                # catSNN.Clamp_q(max = self.clamp_max,q_level =1000)
                layers += [catSNN.QuantizedConv2d(in_channels, out_channels, kernel_size=3, padding=padding, bias=self.bias),nn.BatchNorm2d(out_channels), catSNN.Clamp_q(max = self.clamp_max,q_level =1000),nn.Dropout2d(0.1)]
                in_channels = out_channels
...
class CatVGG_o_test_deep(nn.Module):
    def __init__(self, vgg_name, T, is_noise=False, bias=True):
        super(CatVGG_o_test_deep, self).__init__()
        self.snn = catSNN.spikeLayer(T)
        self.T = T
        self.is_noise = is_noise
        self.bias = bias
        ...
        self.features = self._make_layers(cfg[vgg_name], is_noise)
        self.classifier1 = self.snn.dense((1, 1, 32), 10,bias = True)
