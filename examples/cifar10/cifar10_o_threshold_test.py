...
def threshold_training_snn(input, y_origin,threshold,T,index):
    ...
    for i in range (y_new.shape[1]):
        ...
        #print("before",diff)
        #print(diff)
        #diff_ = torch.sum(torch.logiczal_xor(y.transpose(-4, -5)[i],y_new.transpose(-4, -5)[i]))/mul
...
class CatNet_training(nn.Module):
    ...
    def __init__(self, T):
        ...
        self.classifier1_ =  nn.Linear(1024, 10, bias=True)
...
def main():
    ...
    snn_dataset = SpikeDataset(testset, T = args.T,theta = 0.9)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if correct>correct_:
            correct_ = correct
            torch.save(model.state_dict(), "vggo_9515_t100_.pt")
        scheduler.step()
