...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(32, 192, 1, 1,0, bias=True)
        self.Bn2 = nn.BatchNorm2d(192)
        self.conv3 = nn.Conv2d(192, 192, 3, 1, 1,bias=True,groups = 192)
        self.Bn3 = nn.BatchNorm2d(192)
        self.conv4 = nn.Conv2d(192, 32, 1, 1, 0,bias=True)
...
class CatNet(nn.Module):
    ...
    def __init__(self, T):
        ...
        self.conv2 = snn.conv(32, 192, 1, 1,0,bias=True)
        self.conv3 = snn.conv(192, 192, 3,1,1, bias=True,groups = 192)
        self.conv4 = snn.conv(192, 32, 1,1,0, bias=True)
    ...
    def forward(self, x):
        ...
        x = self.snn.spike(self.conv4(x)+y)
...
def main():
    ...
    dataset2 = datasets.MNIST('../data', train=False,
                       transform=transform)
    snn_dataset = SpikeDataset(dataset2, T = args.T,theta = 0.9999)
    ...
    model = Net().to(device)
    model.load_state_dict(torch.load("mobilev2_basic_new.pt"), strict=False)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(model.state_dict(), "mobilev2_basic_new.pt")
        scheduler.step()
       
    test(model, device, test_loader)
    #torch.save(model.state_dict(), "mobilev2_basic.pt")
    fuse_module(model)
