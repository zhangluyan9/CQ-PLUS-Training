...
class Net(nn.Module):
    def __init__(self):
        ...
        self.conv2 = nn.Conv2d(32, 8, 1, 1,0, bias=True)
        self.Bn2 = nn.BatchNorm2d(8)
        self.conv3 = nn.Conv2d(8, 8, 3, 1, 1,bias=True)
        self.Bn3 = nn.BatchNorm2d(8)
        self.conv4 = nn.Conv2d(8, 32, 1, 1, 0,bias=True)
...
class CatNet(nn.Module):
    ...
    def __init__(self, T):
        ...
        self.conv2 = snn.conv(32, 8, 1, 1,0,bias=True)
        self.conv3 = snn.conv(8, 8, 3,1,1, bias=True)
        self.conv4 = snn.conv(8, 32, 1,1,0, bias=True)
    ...
    def forward(self, x):
        ...
        x = self.snn.spike(self.conv4(x)+y1,theta=0.999)
...
def main():
    ...
    model = Net().to(device)
    model.load_state_dict(torch.load("resnet_bottle_new_1.pt"), strict=False)
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        torch.save(model.state_dict(), "resnet_bottle_new_1.pt")

        scheduler.step()
    test(model, device, test_loader)
    #torch.save(model.state_dict(), "resnet_bottle_new_.pt")
    fuse_module(model)
